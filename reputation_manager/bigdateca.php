...
$variables_metodo = variables_metodo("etiqueta,pag,buscar_rss,fechaIni,fechaFin,categoria,semana_seleccionada,accion_seleccionada,fecha_seleccionada,web,fecha_desde,fecha_hasta,directorio,tipo_seleccionada,rss_id");
...
$directorio= 			$variables_metodo[12];
$tipo_seleccionada= 	$variables_metodo[13];
...
if($categoria!="" && $categoria!="0"){
	...
	$categoria_where = " AND a.id_categoria = '".$categoria."'";
}
...
$sql="SELECT 
		...
		id_categoria
	  FROM ic_rss_coincidencias a, ic_concidencia_directorio b
	  WHERE
	 	b.id_coincidencia=a.id AND
		b.id_directorio='".$directorio."' AND 
...
$sql="SELECT  COUNT(a.id) FROM ic_rss_coincidencias a, ic_concidencia_directorio b WHERE
		b.id_coincidencia=a.id AND
		b.id_directorio='".$directorio."' AND 
		a.id_etiqueta=".$id_etiqueta_mostrar." 
		".$where."
...
list($total_resultados)=$result_totalcoincidencias->fields;

//-------------------------------------------------------------------------------------

$sql="SELECT nombre FROM ic_directorios WHERE id='".$directorio."'";
...
<div style="width:100%; height: 40px; background:url(images/feb-2014/fondo_listado.jpg); margin-bottom: 15px">
	<div style="float:left;">
    	<h2 style="margin: 5px 0 0 30px; font-size: 17px; color:#FFF;">
		<span style="background:url(images/feb-2014/dir.png) no-repeat 0px 3px; padding-left:25px; font-weight:bold"><?=$nombre_dir?>/</span>
        <?=$_SESSION['bdm_etiquetas'][$id_etiqueta_mostrar]['etiqueta']." (".$total_resultados." "._REPUTA_COINCI2.")"?>
        </h2>
    </div>
...
<!-- ///////////////////////////////////////////////////////////// -->
<div id="aplicaciones" style="position:absolute; right:0; margin:-4px 5px 0 0; z-index: 999999; padding:2px; display:none;" class="casillaMenu2 sombra redondeado">   
...
<div id="filtros_form" style="position:absolute; padding-top: 20px; right:0; margin:-4px 5px 0 0; z-index: 999999; width: 211px; display:none;" class="casillaMenu sombra redondeado">   
    ...
    <form action="reputation_manager/bigdateca.php?etiqueta=<?=$id_etiqueta_mostrar?>" method="post" name="buscarRss" id="buscarRss" target="_self">
...
<div style="height:10px;">
    <div style="display:block; width: 250px; float:left; margin:0 0 0 20px;">
...
<div style="width:55%; padding: 0 0 0 20px; overflow:auto; display:block; float:left;" id="listado">
<?php
...
while(!$result_coincidencias_finales->EOF){
	...
	list($id,$id_rss,$cargue,$marcada,$positivo,$negativo,$bloqueada,$intervenido,$detalle_intervenido,
	     $cat_id,$titulo,$contenido,$link,$semana,$neutro,$id_source,$detalle_intervenido,
		 $fecha_registro,$tipo_mencion,$usuario_medio,$media_url,$ubicacion,$info_adicional,$nom_rss)=select_format($result_coincidencias_finales->fields);
...
<table width="100%" border="0" cellspacing="0" cellpadding="1" style="margin-bottom:10px;" id="filaElemento<?=$cont?>">
  <tr>
    ...
    <input type="hidden" id="media_<?=$id?>" value="<?=$media?>" />
...
<?php
	...
	echo '<input type="hidden" name="final" id="final" value="'.$id.'" />';
	...
	if($total_resultados>$max_pagi){
		...
		paginas($total_resultados, 
				$pag, 
				$max_pagi, 
				"reputation_manager/bigdateca.php?etiqueta=".$id_etiqueta_mostrar."".
...
<div style="width:40%; padding: 0 0 0 30px; overflow:hidden; display:block; float:left;" id="mini">
