...
$sql="SELECT  COUNT(*) FROM ic_rss_coincidencias a WHERE
		a.id_etiqueta=".$id_etiqueta_mostrar." AND
		a.intervenido <> 'N' 
		".$where."
		".$categoria_where."";
...
<div style="width:100%; height: 40px; background:url(images/feb-2014/fondo_listado.jpg); margin-bottom: 15px">
	<div style="float:left;">
		...
		<strong><?=_REPUTA_FEEDB?></strong> <?="(".$_SESSION['bdm_etiquetas'][$id_etiqueta_mostrar]['etiqueta'].")"?>
        </h2>
    </div>
...
<div id="filtros_form" style="position:absolute; padding-top: 20px; right:0; margin:-4px 5px 0 0; z-index: 999999; width: 211px; display:none;" class="casillaMenu sombra redondeado">   
    ...
    <form action="reputation_manager/mis_feedback.php?etiqueta=<?=$id_etiqueta_mostrar?>" method="post" name="buscarRss" id="buscarRss" target="_self">
...
while(!$result_coincidencias_finales->EOF){
	...
	list($id,$id_rss,$cargue,$marcada,$positivo,$negativo,$bloqueada,$intervenido,$id_categoria,		
		 $titulo,$contenido,$link,$semana,$neutro,$id_source,$detalle_intervenido,
...
<table width="100%" border="0" cellspacing="0" cellpadding="1" style="margin-bottom:10px;" id="filaElemento<?=$cont?>">
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td>    
       <strong><div style="color:#999999;"><?=$contenido?></div></strong>
       <div><?=$media?></div>
...
<?php
	...
	if($total_resultados>$max_pagi){
		...
		paginas($total_resultados, 
				$pag, 
				$max_pagi, 
				"reputation_manager/mis_feedback.php?etiqueta=".$id_etiqueta_mostrar."".
